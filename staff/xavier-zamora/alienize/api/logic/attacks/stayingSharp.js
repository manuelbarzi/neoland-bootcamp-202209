...
function stayingSharp(attack, defend){
    return GameAliens.findById({ _id: attack })
    .then(alienAttack => {
        const especialAttack = alienAttack.especialAttack
        return GameAliens.findById({ _id: defend })
            .then(alienDefend => {
                ...
                const damage = especialAttack - especialDefense 
