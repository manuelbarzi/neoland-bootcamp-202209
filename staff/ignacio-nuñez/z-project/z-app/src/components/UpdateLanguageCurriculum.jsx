...
function UpdateLanguageCurriculum({ className, onUpdateLanguageCurriculumClose, onUpdateLanguageCurriculum, curriculumLanguageData }) {
    const [languages, setLanguages] = useState(curriculumLanguageData.languages)
    ...
    const updateLanguageCurriculumHandler = event => {
        ...
        try {
            updateCurriculum(sessionStorage.token, curriculumLanguageData.curriculumId, curriculumLanguageData.curriculumUserId, { languages })
                .then(() => onUpdateLanguageCurriculum())
                .catch(error => {
    ...
    const closeLanguageCurriculumHandler = () => {
        onUpdateLanguageCurriculumClose()
    }
    ...
    return <div className="z-20 fixed w-screen h-screen bg-[#aaaa] inset-y-0" onClick={closeLanguageCurriculumHandler}>
        <article onClick={event => event.stopPropagation()} className={`shadow-lg shadow-slate-400 w-[95%] h-fit bg-white border-2 p-4 rounded-xl ${className ? className : ""}`}>
            <div className="flex flex-col items-center">
                <div className="grid w-full items-center grid-cols-12">
                    ...
                    <button className="border-2 w-fit h-fit justify-self-end col-start-12 col-end-13 px-2 py-1 rounded-xl" onClick={closeLanguageCurriculumHandler}>X</button>
                </div>
                <hr className="w-full mt-3.5" />
                <form onSubmit={updateLanguageCurriculumHandler} className="flex flex-col items-center w-full mt-2 gap-2">
...
export default UpdateLanguageCurriculum