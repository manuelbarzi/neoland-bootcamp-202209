...
function VisiblePosts({ postChanged, editingPost, deletingPost }) {
    ...
    useEffect(() => {
        if(lastPageRequested < page) {
            try {
                retrievePosts(sessionStorage.token, page)
                    .then(postsFromLogic => {
    ...
    useEffect(() => {
        try {
            retrievePosts(sessionStorage.token, 1, (6*page))
                .then(posts=> {
                    setPosts(posts)
                })
                .catch(error => alert(error.message))
        ...
        {posts.map(post => {
            return <article key={post.id} className=" shadow-sm shadow-slate-600 bg-emerald-200 flex flex-col mt-3.5 border-2 p-4 w-96 rounded-xl">
                <div className="flex justify-between z-10">
                    <Link className='font-semibold' to={`/user/${post.user.id}`}>{post.user.name}</Link>
