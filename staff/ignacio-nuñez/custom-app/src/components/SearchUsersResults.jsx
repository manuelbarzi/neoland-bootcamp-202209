...
function SearchUsersResults(props) {
    ...
    return <div className="border-2 shadow-sm shadow-slate-600 flex flex-col col-start-1 col-end-4 bg-teal-300 rounded-xl">
        {searchedUsers && props.usersFounded.length > 0 ? <ul className="flex flex-col p-2 rounded-xl">
            {searchedUsers.map(searchedUser => {
                return <li key={searchedUser.userId} className="flex bg-slate-100 mt-2 border-2 rounded-xl">
                    <Link to={`/user/${searchedUser.userId}`} className="p-2 h-full w-full">{searchedUser.name}</Link></li>
            })}
