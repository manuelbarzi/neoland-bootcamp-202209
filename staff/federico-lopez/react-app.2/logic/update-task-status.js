function updateTaskStatus(userEmail, taskId, newStatus) {
  ...
  if (!IS_EMAIL_REGEX.test(userEmail))
    throw new Error("userEmail is not valid");

  if (typeof taskId !== "string") throw new TypeError("taskId is not a string");
  if (taskId.length < 6 || !taskId.startsWith("task-"))
    throw new Error("invalid taskId");

  if (typeof newStatus !== "string")
    throw new TypeError("newStatus is not a string");
  if (newStatus !== "todo" && newStatus !== "doing" && newStatus !== "done")
    throw new Error("newStatus is not valid");
