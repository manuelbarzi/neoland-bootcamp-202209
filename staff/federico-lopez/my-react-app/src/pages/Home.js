...
function Home() {
  ...
  return (
    <main className="overflow-hidden">
      ...
      {posts && (
        <div className="flex flex-col items-center gap-2 py-[2rem]">
          {posts.map((post) => (
            <article
              key={post.id}
              className="border rounded-xl w-[50%] flex flex-col p-5"
            >
      ...
      <footer className="fixed bg-[white] w-full h-[2rem] bottom-0 flex justify-center">
      ...
      {createPostVisible && (
        <CreatePost onCreated={handlePostCreated} onClose={closeCreatePost} />
      )}
