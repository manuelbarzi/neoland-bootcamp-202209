...
export default function Alert({ message, level = 'error', onClose }) {
    ...
    return <div className='bg-[#aaa] fixed top-0 h-full w-full flex flex-col justify-center items-center overflow-hidden' onClick={onClose}>
        <div className={`p-5 rounded-xl flex flex-col items-end gap-2 ${level === 'fatal' ? 'bg-[red]' : 'bg.-white dark:bg-black'} text-black dark:text-white2 border-4 border-[${color}]`} onClick={event => event.stopPropagation()}>
            <p>{message}</p>
