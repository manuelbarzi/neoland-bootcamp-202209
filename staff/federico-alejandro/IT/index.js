...
document.onkeydown = function (event) {
    ...
    if (key === 'ArrowUp' && !(keyPressed.includes('ArrowUp')))
        intervalId = setInterval(() => {
            crazy.move(crazy.position.x, crazy.position.y + 1 )
            keyPressed[0] = 'ArrowUp'

            crazy.render()
        }, 2);
    else if (key === 'ArrowDown' && keyPressed !== 'ArrowDown')
        intervalId = setInterval(() => {
            crazy.move(crazy.position.x, crazy.position.y - 0.5)

            crazy.render()
        }, 2)
    else if (key === 'ArrowLeft' && keyPressed !== 'ArrowLeft')
        intervalId = setInterval(() => {
            crazy.move(crazy.position.x - 10, crazy.position.y - 0.5)

            crazy.render()
        }, 2)
    else if (key === 'ArrowRight' && keyPressed !== 'ArrowRight')
        intervalId = setInterval(() => {
            crazy.move(crazy.position.x + 10, crazy.position.y - 0.5)
