...
export default function EditPost({ onUpdated, onClose, postId }) {
    const [post, setPost] = useState()
    const [visibility, setVisibility] = useState()

    useEffect(() => {
        try {
            retrievePost(sessionStorage.userId, postId)
                .then((postRetrieved) => {
                    setPost(postRetrieved)
                    setVisibility(postRetrieved.visibility)
                })
    ...
    const submitUpdatePost = event => {
        ...
        try {
            updatePost(sessionStorage.userId, postId, text, visibility)
                .then(() => onUpdated())
                .catch((error) => alert(error.message))
        } catch (error) {
            alert(error.message)
