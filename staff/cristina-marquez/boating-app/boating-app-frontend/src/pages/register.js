...
function Register() {
  ...
  const registerBirthDate = (event) => {
    ...
    if (birthDateObject.getTime() > now.getTime()) {
      setToastMessage("Invalid date, please select your date of birth");
      setIsToastActive(true);
      event.value = "";
    } else {
      setBirthDate(event.value);
    }
  };
  ...
  const onRegisterFinish = async (event) => {
    ...
    try {
      ...
      console.log(
        "INFO",
        `User ${decodedToken.name} successfully registered and logged in`
      );
